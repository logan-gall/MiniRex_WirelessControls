...
def draw_gui_elements(selected_serial_port, selected_baud_rate, selected_joystick_index, serial_ports, baud_rates, joystick_names, telemetry_data):
    ...
    joystick_name = joystick_names[selected_joystick_index + 1] if selected_joystick_index is not None else "None"
...
def main():
    ...
    selected_baud_rate = 921600  # Default baud rate set to 921600
    ...
    axis_channel_map = {}  # e.g., {'axis_0': {'channel': 1, 'invert': False}, ...}
    button_channel_map = {}  # e.g., {'button_0': {'channel': 2, 'invert': False}, ...}
    hat_channel_map = {'x': {}, 'y': {}}  # e.g., {'x': {'hat_0_x': {'channel': 3, 'invert': False}}, 'y': {...}}
    ...
    serial_read_thread = threading.Thread(target=serial_thread_func)
    ...
    ser_write_thread = threading.Thread(target=serial_write_thread_func)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if dropdown_active:
                    ...
                    index = baud_rates.index(selected_baud_rate) if selected_baud_rate in baud_rates else -1
